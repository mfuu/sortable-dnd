  ...
  return {
    ...
    _onMultiChange: debounce(function ({ dragEl, rootEl, target, e, evt }) {
      ...
      if (!lastChild(rootEl) || (target === rootEl && multiDiffer.from.group !== rootEl)) {
        ...
        // on-remove
        multiDiffer.from.sortable._dispatchEvent('onRemove', { ..._emitMultiDiffer(), event: e, originalEvent: evt })
        // on-add
        this._dispatchEvent('onAdd', { ..._emitMultiDiffer(), event: e, originalEvent: evt })
        ...
        if (clientX > left && clientX < right && clientY > top && clientY < bottom) {
          ...
          if (multiDiffer.from.group !== multiDiffer.to.group) { 
            ...
            // on-change
            this._dispatchEvent('onChange', { ..._emitMultiDiffer(), event: e, originalEvent: evt })
          }
          this._animate()
        }
      }
    ...
    _onMultiDrop: function ({ fromGroup, fromSortable, dragEl, rootEl, evt }) {
      ...
      const params = { ..._emitMultiDiffer(), changed, event: evt, originalEvent: evt }
